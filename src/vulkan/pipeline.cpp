...
bool VkRendererPipeline::init(VkRenderData& renderData, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline,
	std::string vertexShaderFilename, std::string fragmentShaderFilename)
{
	VkShaderModule vertexModule = VkRendererShader::loadShader(renderData.rendererVkbDevice.device, vertexShaderFilename);
	VkShaderModule fragmentModule = VkRendererShader::loadShader(renderData.rendererVkbDevice.device, fragmentShaderFilename);

	if(vertexModule == VK_NULL_HANDLE || fragmentModule == VK_NULL_HANDLE)
	{
		Logger::log(1, "%s error: could not load shaders", __FUNCTION__);
		return false;
	}
	...
	viewport.x = 0.0f;
	viewport.y = 0.0f;
	viewport.width = static_cast<float>(renderData.rendererVkbSwapchain.extent.width);
	viewport.height = static_cast<float>(renderData.rendererVkbSwapchain.extent.height);
	viewport.minDepth = 0.0f;
	viewport.maxDepth = 1.0f;

	VkRect2D scissor{};
	scissor.offset = { 0, 0 };
	scissor.extent = renderData.rendererVkbSwapchain.extent;
	...
	pipelineCreateInfo.renderPass = renderData.rendererRenderpass;
	pipelineCreateInfo.subpass = 0;
	pipelineCreateInfo.basePipelineHandle = VK_NULL_HANDLE;

	if (vkCreateGraphicsPipelines(renderData.rendererVkbDevice.device, VK_NULL_HANDLE, 1, &pipelineCreateInfo, nullptr, &pipeline) != VK_SUCCESS) {
	...
	vkDestroyShaderModule(renderData.rendererVkbDevice.device, fragmentModule, nullptr);
	vkDestroyShaderModule(renderData.rendererVkbDevice.device, vertexModule, nullptr);

	return true;
}

void VkRendererPipeline::cleanup(VkRenderData& renderData, VkPipeline& pipeline)
{
