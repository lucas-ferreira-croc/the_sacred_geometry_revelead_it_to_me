...
bool VkRendererTexture::loadTexture(VkRenderData& renderData, VkTextureData& textureData, std::string textureFilename)
{
    int texWidth;
    int texHeight;
    int numberOfChannels;

    unsigned char* texture = stbi_load(textureFilename.c_str(), &texWidth, &texHeight, &numberOfChannels, STBI_rgb_alpha);
    ...
    if (vmaCreateImage(renderData.rendererAllocator, &imageInfo, &imageAllocInfo, &textureData.textureImage, &textureData.textureImageAllocation, nullptr) != VK_SUCCESS)
    {
    ...
    if (vmaCreateBuffer(renderData.rendererAllocator, &stagingBufferInfo, &stagingAllocInfo, &stagingBuffer, &stagingBufferAlloc, nullptr) != VK_SUCCESS) 
    {
    ...
    void* data;
    vmaMapMemory(renderData.rendererAllocator, stagingBufferAlloc, &data);
    std::memcpy(data, texture, static_cast<uint32_t>(imageSize));
    vmaUnmapMemory(renderData.rendererAllocator, stagingBufferAlloc);
    ...
    stagingBufferTransferBarrier.image = textureData.textureImage;
    ...
    stagingBufferShaderBarrier.image = textureData.textureImage;
    ...
    vkCmdCopyBufferToImage(stagingCommandBuffer, stagingBuffer, textureData.textureImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &stagingBufferCopy);
    ...
    if (vkCreateFence(renderData.rendererVkbDevice.device, &fenceInfo, nullptr, &stagingBufferFence) != VK_SUCCESS) 
    {
        Logger::log(1, "%s error: failed to create staging buffer fence\n", __FUNCTION__);
        return false;
    }

    if (vkResetFences(renderData.rendererVkbDevice.device, 1, &stagingBufferFence) != VK_SUCCESS) 
    {
        Logger::log(1, "%s error: staging buffer fence reset failed\n", __FUNCTION__);
        return false;
    }

    if (vkQueueSubmit(renderData.rendererGraphicsQueue, 1, &submitInfo, stagingBufferFence) != VK_SUCCESS)
    {
        Logger::log(1, "%s error: failed to submit staging buffer copy command buffer\n", __FUNCTION__);
        return false;
    }

    if (vkWaitForFences(renderData.rendererVkbDevice.device, 1, &stagingBufferFence, VK_TRUE, UINT64_MAX) != VK_SUCCESS)
    {
        Logger::log(1, "%s error: waiting for staging buffer copy fence failed\n", __FUNCTION__);
        return false;
    }

    vkDestroyFence(renderData.rendererVkbDevice.device, stagingBufferFence, nullptr);
    CommandBuffer::cleanup(renderData, stagingCommandBuffer);
    vmaDestroyBuffer(renderData.rendererAllocator, stagingBuffer, stagingBufferAlloc);

    /* image view and sampler */
    VkImageViewCreateInfo texViewInfo{};
    texViewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
    texViewInfo.image = textureData.textureImage;
    ...
    if (vkCreateImageView(renderData.rendererVkbDevice.device, &texViewInfo, nullptr, &textureData.textureImageView) != VK_SUCCESS) 
    {
    ...
    if (vkCreateSampler(renderData.rendererVkbDevice.device, &texSamplerInfo, nullptr, &textureData.textureSampler) != VK_SUCCESS)
    {
    ...
    if (vkCreateDescriptorSetLayout(renderData.rendererVkbDevice.device, &textureCreateInfo, nullptr, &textureData.textureDescriptorLayout) != VK_SUCCESS)
    {
    ...
    if (vkCreateDescriptorPool(renderData.rendererVkbDevice.device, &descriptorPool, nullptr, &textureData.textureDescriptorPool) != VK_SUCCESS)
    {
    ...
    descriptorAllocateInfo.descriptorPool = textureData.textureDescriptorPool;
    descriptorAllocateInfo.descriptorSetCount = 1;
    descriptorAllocateInfo.pSetLayouts = &textureData.textureDescriptorLayout;

    if (vkAllocateDescriptorSets(renderData.rendererVkbDevice.device, &descriptorAllocateInfo, &textureData.textureDescriptorSet) != VK_SUCCESS)
    {
    ...
    descriptorImageInfo.imageView = textureData.textureImageView;
    descriptorImageInfo.sampler = textureData.textureSampler;

    VkWriteDescriptorSet writeDescriptorSet{};
    writeDescriptorSet.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writeDescriptorSet.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    writeDescriptorSet.dstSet = textureData.textureDescriptorSet;
...
void VkRendererTexture::cleanup(VkRenderData& renderData, VkTextureData& textureData) {
    vkDestroyDescriptorPool(renderData.rendererVkbDevice.device, textureData.textureDescriptorPool, nullptr);
    vkDestroyDescriptorSetLayout(renderData.rendererVkbDevice.device, textureData.textureDescriptorLayout, nullptr);
    vkDestroySampler(renderData.rendererVkbDevice.device, textureData.textureSampler, nullptr);
    vkDestroyImageView(renderData.rendererVkbDevice.device, textureData.textureImageView, nullptr);
    vmaDestroyImage(renderData.rendererAllocator, textureData.textureImage, textureData.textureImageAllocation);
}