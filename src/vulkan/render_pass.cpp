...
bool VkRendererRenderPass::init(VkRenderData& renderData)
{
	VkAttachmentDescription colorAtt{};
	colorAtt.format = renderData.rendererVkbSwapchain.image_format;
	...
	depthAtt.flags = 0;
	depthAtt.format = renderData.rendererDepthFormat;
	...
	if (vkCreateRenderPass(renderData.rendererVkbDevice.device, &renderPassInfo, nullptr, &renderData.rendererRenderpass) != VK_SUCCESS) {
...
void VkRendererRenderPass::cleanup(VkRenderData& renderData)
{
	vkDestroyRenderPass(renderData.rendererVkbDevice.device, renderData.rendererRenderpass, nullptr);
}
